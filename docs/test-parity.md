...
| Middleware transformation hooks | rename and argument injection with the injected value absent from the audit, type conversion required in `on_resolve`, a rename that cannot route around a deny, resolve failures and renames onto nothing, call substitution and its short-circuit, substituted-result validation, chained result rewriting, redaction versus the output schema, catalog transformation on all three listings, listing failure mapping, `wrap_sink` progress-before-response ordering, reverse wrapping order, swallowed and duplicate completions, wrap failure, pass-through defaults, and two channels through one wrapper on two threads |

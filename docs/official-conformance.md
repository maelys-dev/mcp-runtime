...
URL. Maelys MCP Runtime 0.3.x intentionally exposes a local stdio transport, so the
...
against the 2026-07-28 schema, list tools, return text/image/audio/embedded/mixed
content, report tool errors, complete a two-round elicitation, and respect declared
client capabilities.

This is deliberately a partial conformance result. The frozen 2026-07-28 requirements
also mandate resources, prompts, request-scoped SSE, subscriptions and further MRTR
scenarios outside the current tool-scoped module. The runtime does not implement or
